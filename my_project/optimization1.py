""" Code for Discharging strategy when battery_uesd=1  ie when cost of getting Energy
from battery is less when compared to market and grid"""
...
def compute_metrics(solar_size, wind_size, battery_size, df, config_list, colocation_option):
    ...
    for i in range(len(df)):
        ...
        discharge =0
        battery_used = 0
        if battery_cost_per_MWh < min(market_price[i], grid_cost_val):
            battery_used = 1
